    ...
    if(children_pids[indx-1]==0) {
        ...
        if (strcmp(mode, "sep")==0) {
            ...
            if (paste_pid == 0) {
                ...
                int fp_paste = open(paste_file, O_RDWR | O_TRUNC | O_CREAT, S_IRWXU | S_IRWXO);
                char buff[10];
                snprintf(buff, 10, "%d\n", children_no);
                ...
                fp_paste = open(paste_file, O_RDWR | O_TRUNC);
                flock(fp_paste, LOCK_EX);
                snprintf(buff, 10, "%d\n", 0);
                write(fp_paste, buff, strlen(buff));
                flock(fp_paste, LOCK_UN);
                close(fp_paste);
